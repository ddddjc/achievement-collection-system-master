...
public class TalentTextBookController {

    @Resource
    TalentTextbookService talentTextbookService;

    @ApiOperation(value = "提交一条记录",notes = "names是所有成功上传的文件名")
    @PostMapping(value = "/textbook")
    ResponseResult insert(@RequestBody TalentTextbook record){
        return talentTextbookService.insertSelective(record);
    }


    @ApiOperation(value = "根据唯一id更新记录",notes = "如果用户未新添加文件，names传空即可，更新文件只用传新文件的names")
    @PostMapping("/textbook/pu")
    ResponseResult update(@RequestBody TalentTextbook record){
        record.setPublishedDate(DataLat.set(record.getPublishedDate()));
        record.setSubmitTime(DataLat.set(record.getSubmitTime()));

        return talentTextbookService.updateByPrimaryKeySelective(record);
    }

    @ApiOperation("根据老师工号查询之前提交的全部记录")
    @GetMapping("/textbook")
    ResponseResult records(String username){
        return talentTextbookService.findByUsername(username);
    }
    ...
    ResponseResult delete(@RequestBody HashMap<String,String> Id){
        return talentTextbookService.deleteById(Long.parseLong(Id.get("Id")));
    }

    @ApiOperation(value = "根据唯一id查询单条记录",notes = "包括单条记录和所有附件名称")
    @GetMapping("/textbookRecord")
    ResponseResult record(Long recordId){
        return talentTextbookService.findByRecordId(recordId);
    }

    @ApiOperation(value = "查询所有记录")
    @GetMapping("/talentTextBookRecords")
    ResponseResult find(){
        return talentTextbookService.find();
    }
}
