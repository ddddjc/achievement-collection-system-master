package com.emc.achievement.controller;


import com.emc.achievement.entity.ResponseResult;
import com.emc.achievement.entity.TalentProject;
import com.emc.achievement.service.TalentProjectService;
import com.emc.achievement.util.DataLat;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiImplicitParam;
import io.swagger.annotations.ApiOperation;
import lombok.extern.slf4j.Slf4j;
import org.springframework.web.bind.annotation.*;

import javax.annotation.Resource;
import java.util.HashMap;

@Slf4j
@Api(tags = "人才培养-教研项目")
@RestController
@RequestMapping("/talent")
public class TalentProjectController {
    @Resource
    TalentProjectService talentProjectService;

    @ApiOperation(value = "提交一条记录",notes = "names是所有成功上传的文件名")
    @PostMapping(value = "/project")
    ResponseResult insert(@RequestBody TalentProject record){
        log.info("提交一条记录");
        return talentProjectService.insertSelective(record);
    }


    @ApiOperation(value = "根据唯一id更新记录",notes = "如果用户未新添加文件，names传空即可，更新文件只用传新文件的names")
    @PostMapping("/project/pu")
    ResponseResult update(@RequestBody TalentProject record){
        log.info("put");
        record.setBornAt(DataLat.set(record.getBornAt()));
        record.setSubmitTime(DataLat.set(record.getSubmitTime()));
        record.setEndTime(DataLat.set(record.getEndTime()));
        return talentProjectService.updateByPrimaryKeySelective(record);
    }

    @ApiOperation("根据老师工号查询之前提交的全部记录")
    @GetMapping("/project")
    ResponseResult records(String username){
        log.info("get");
        return talentProjectService.findByUsername(username);
    }

    @ApiOperation(value = "删除记录")
    @ApiImplicitParam(name = "Id",value = "Id")
    @PostMapping("/project/del")
    ResponseResult delete(@RequestBody HashMap<String,String> Id){
        log.info("del");
        System.out.println(Id.get("Id"));
        return talentProjectService.deleteById(Long.parseLong(Id.get("Id")));
    }

    @ApiOperation(value = "根据唯一id查询单条记录",notes = "包括单条记录和所有附件名称")
    @GetMapping("/projectRecord")
    ResponseResult record(Long recordId){
        log.info("/projectRecord");
        return talentProjectService.findByRecordId(recordId);
    }

    @ApiOperation(value = "查询所有记录")
    @GetMapping("/talentProjectRecords")
    ResponseResult find(){
        log.info("/talentProjectRecords");
        return talentProjectService.find();
    }
}
