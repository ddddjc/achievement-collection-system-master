...
import com.emc.achievement.mapper.ResearchProjectMapper;
...
public class ResearchProjectServiceImpl implements ResearchProjectService {
     ...
     */
    @Override
    public ResponseResult insertSelective(ResearchProject record) {
        log.info("======= 插入ResearchProject数据 ======="+record);
        record.setId(idGenerator.nextId());
        record.setSubmitTime(new Date());
        if (researchProjectMapper.insertSelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 插入ResearchProject成功 =======");
            return ResponseResult.ofSuccess("添加成功");
        }
        log.error("======= 插入ResearchProject失败 =======");
     ...
     */
    @Override
    public ResponseResult updateByPrimaryKeySelective(ResearchProject record) {
        log.info("======= 更新ResearchProject数据 ======="+record);
        if (researchProjectMapper.updateByPrimaryKeySelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 更新ResearchProject成功 =======");
            return ResponseResult.ofSuccess("更新成功");
        }
        log.error("======= 更新ResearchProject失败 =======");
    ...
    public ResponseResult findByUsername(String username) {
        log.info("======= 查找老师的所有ResearchProject记录 ======="+username);
        List<ResearchProject> records = researchProjectMapper.findByUsername(username);
    ...
    public ResponseResult findByRecordId(Long recordId) {
        log.info("======= 查找单条ResearchProject记录 ======="+recordId);
        HashMap<String, Object> map = new HashMap<>();
        map.put("record", researchProjectMapper.findById(recordId));
    ...
    public ResponseResult find() {
        log.info("======= 导出ResearchProject全部记录 =======");
        return ResponseResult.ofSuccess("查询全部记录",researchProjectMapper.find());
    }

    @Override
    public ResponseResult deleteById(Long Id) {
        if(researchProjectMapper.deleteById(Id) == 1){
