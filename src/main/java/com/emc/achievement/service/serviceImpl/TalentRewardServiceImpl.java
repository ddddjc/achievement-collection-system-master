...
import com.emc.achievement.mapper.TalentRewardMapper;
...
@Slf4j
@Service
public class TalentRewardServiceImpl implements TalentRewardService {

    @Resource
    private TalentRewardMapper talentRewardMapper;
    @Resource
     ...
     */
    @Override
    public ResponseResult insertSelective(TalentReward record) {
        log.info("======= 插入TalentReward数据 ======="+record);
        record.setId(idGenerator.nextId());
        record.setSubmitTime(new Date());
        if (talentRewardMapper.insertSelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 插入TalentReward成功 =======");
            return ResponseResult.ofSuccess("添加成功");
        }
        log.error("======= 插入TalentReward失败 =======");
     ...
     */
    @Override
    public ResponseResult updateByPrimaryKeySelective(TalentReward record) {
        log.info("======= 更新TalentReward数据 ======="+record);
        if (talentRewardMapper.updateByPrimaryKeySelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 更新TalentReward成功 =======");
            return ResponseResult.ofSuccess("更新成功");
        }
        log.error("======= 更新TalentReward失败 =======");
    ...
    public ResponseResult findByUsername(String username) {
        log.info("======= 查找老师的所有TalentReward记录 ======="+username);
        List<TalentReward> records = talentRewardMapper.findByUsername(username);
    ...
    public ResponseResult findByRecordId(Long recordId) {
        log.info("======= 查找TalentReward单条记录 ======="+recordId);
        HashMap<String, Object> map = new HashMap<>();
        map.put("record", talentRewardMapper.findById(recordId));
    ...
    public ResponseResult find() {
        log.info("======= 导出TalentReward全部记录 =======");
        return ResponseResult.ofSuccess("查询全部记录",talentRewardMapper.find());
    }

    @Override
    public ResponseResult deleteById(Long Id) {
        if(talentRewardMapper.deleteById(Id) == 1){
