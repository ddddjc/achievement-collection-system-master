...
import com.emc.achievement.mapper.TalentCourseMapper;
...
@Slf4j
@Service
public class TalentCourseServiceImpl implements TalentCourseService {

    @Resource
    private TalentCourseMapper talentCourseMapper;
    @Resource
     ...
     */
    @Override
    public ResponseResult insertSelective(TalentCourse record) {
        log.info("======= 插入TalentCourse数据 ======="+record);
        record.setId(idGenerator.nextId());
        record.setSubmitTime(new Date());
        if (talentCourseMapper.insertSelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 插入TalentCourse成功 =======");
            return ResponseResult.ofSuccess("添加成功");
        }
        log.error("======= 插入TalentCourse失败 =======");
     ...
     */
    @Override
    public ResponseResult updateByPrimaryKeySelective(TalentCourse record) {
        log.info("======= 更新TalentCourse数据 ======="+record);
        if (talentCourseMapper.updateByPrimaryKeySelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 更新TalentCourse成功 =======");
            return ResponseResult.ofSuccess("更新成功");
        }
        log.error("======= 更新TalentCourse失败 =======");
    ...
    public ResponseResult findByUsername(String username) {
        log.info("======= 查找老师的所有TalentCourse记录 ======="+username);
        List<TalentCourse> records = talentCourseMapper.findByUsername(username);
    ...
    public ResponseResult findByRecordId(Long recordId) {
        log.info("======= 查找单条TalentCourse记录 ======="+recordId);
        HashMap<String, Object> map = new HashMap<>();
        map.put("record", talentCourseMapper.findById(recordId));
    ...
    public ResponseResult find() {
        log.info("======= 导出TalentCourse全部记录 =======");
        return ResponseResult.ofSuccess("查询全部记录",talentCourseMapper.find());
    }

    @Override
    public ResponseResult deleteById(Long Id) {
        if(talentCourseMapper.deleteById(Id) == 1){
