...
import com.emc.achievement.mapper.ResearchReportMapper;
...
public class ResearchReportServiceImpl implements ResearchReportService {
     ...
     */
    @Override
    public ResponseResult insertSelective(ResearchReport record) {
        log.info("======= 插入ResearchReport数据 ======="+record);
        record.setId(idGenerator.nextId());
        record.setSubmitTime(new Date());
        if (researchReportMapper.insertSelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 插入ResearchReport成功 =======");
            return ResponseResult.ofSuccess("添加成功");
        }
        log.error("======= 插入ResearchReport失败 =======");
     ...
     */
    @Override
    public ResponseResult updateByPrimaryKeySelective(ResearchReport record) {
        log.info("======= 更新ResearchReport数据 ======="+record);
        if (researchReportMapper.updateByPrimaryKeySelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 更新ResearchReport成功 =======");
            return ResponseResult.ofSuccess("更新成功");
        }
        log.error("======= 更新ResearchReport失败 =======");
    ...
    public ResponseResult findByUsername(String username) {
        log.info("======= 查找老师的所有ResearchReport记录 ======="+username);
        List<ResearchReport> records = researchReportMapper.findByUsername(username);
    ...
    public ResponseResult findByRecordId(Long recordId) {
        log.info("======= 查找单条ResearchReport记录 ======="+recordId);
        HashMap<String, Object> map = new HashMap<>();
        map.put("record", researchReportMapper.findById(recordId));
    ...
    public ResponseResult find() {
        log.info("======= 导出ResearchReport全部记录 =======");
        return ResponseResult.ofSuccess("查询全部记录",researchReportMapper.find());
    }

    @Override
    public ResponseResult deleteById(Long Id) {
        if(researchReportMapper.deleteById(Id) == 1){
