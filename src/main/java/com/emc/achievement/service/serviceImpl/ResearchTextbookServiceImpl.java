...
import com.emc.achievement.mapper.ResearchTextbookMapper;
...
@Slf4j
@Service
public class ResearchTextbookServiceImpl implements ResearchTextbookService {

    @Resource
    private ResearchTextbookMapper researchTextbookMapper;
    @Resource
     ...
     */
    @Override
    public ResponseResult insertSelective(ResearchTextbook record) {
        log.info("======= 插入ResearchTextbook数据 ======="+record);
        record.setId(idGenerator.nextId());
        record.setSubmitTime(new Date());
        System.out.println(record.toString());
        if (researchTextbookMapper.insertSelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 插入ResearchTextbook成功 =======");
            return ResponseResult.ofSuccess("添加成功");
        }
        log.error("======= 插入ResearchTextbook失败 =======");
     ...
     */
    @Override
    public ResponseResult updateByPrimaryKeySelective(ResearchTextbook record) {
        log.info("======= 更新ResearchTextbook数据 ======="+record);
        if (researchTextbookMapper.updateByPrimaryKeySelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 更新ResearchTextbook成功 =======");
            return ResponseResult.ofSuccess("更新成功");
        }
        log.error("======= 更新ResearchTextbook失败 =======");
    ...
    public ResponseResult findByUsername(String username) {
        log.info("======= 查找老师的所有ResearchTextbook记录 ======="+username);
        List<ResearchTextbook> records = researchTextbookMapper.findByUsername(username);
    ...
    public ResponseResult findByRecordId(Long recordId) {
        log.info("======= 查找单条ResearchTextbook记录 ======="+recordId);
        HashMap<String, Object> map = new HashMap<>();
        map.put("record", researchTextbookMapper.findById(recordId));
    ...
    public ResponseResult find() {
        log.info("======= 导出ResearchTextbook全部记录 =======");
        return ResponseResult.ofSuccess("查询全部记录",researchTextbookMapper.find());
    }

    @Override
    public ResponseResult deleteById(Long Id) {
        if(researchTextbookMapper.deleteById(Id) == 1){
