...
import com.emc.achievement.mapper.ServiceJudgmentMapper;
...
@Slf4j
@Service
public class ServiceJudgmentServiceImpl implements ServiceJudgmentService {

    @Resource
    private ServiceJudgmentMapper serviceJudgmentMapper;
    @Resource
     ...
     */
    @Override
    public ResponseResult insertSelective(ServiceJudgment record) {
        log.info("======= 插入ServiceJudgment数据 ======="+record);
        record.setId(idGenerator.nextId());
        record.setSubmitTime(new Date());
        if (serviceJudgmentMapper.insertSelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 插入ServiceJudgment成功 =======");
            return ResponseResult.ofSuccess("添加成功");
        }
        log.error("======= 插入ServiceJudgment失败 =======");
     ...
     */
    @Override
    public ResponseResult updateByPrimaryKeySelective(ServiceJudgment record) {
        log.info("======= 更新ServiceJudgment数据 ======="+record);
        if (serviceJudgmentMapper.updateByPrimaryKeySelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 更新ServiceJudgment成功 =======");
            return ResponseResult.ofSuccess("更新成功");
        }
        log.error("======= 更新ServiceJudgment失败 =======");
    ...
    public ResponseResult findByUsername(String username) {
        log.info("======= 查找老师的所有ServiceJudgment记录 ======="+username);
        List<ServiceJudgment> records = serviceJudgmentMapper.findByUsername(username);
    ...
    public ResponseResult findByRecordId(Long recordId) {
        log.info("======= 查找单条ServiceJudgment记录 ======="+recordId);
        HashMap<String, Object> map = new HashMap<>();
        map.put("record", serviceJudgmentMapper.findById(recordId));
    ...
    public ResponseResult find() {
        log.info("======= 导出ServiceJudgment全部记录 =======");
        return ResponseResult.ofSuccess("查询全部记录",serviceJudgmentMapper.find());
    }

    @Override
    public ResponseResult deleteById(Long Id) {
        if(serviceJudgmentMapper.deleteById(Id) == 1){
