...
import com.emc.achievement.mapper.ResearchMeetingMapper;
...
/**
*@author: hxq
*@date: 2020/10/7 下午4:20
*@description: 科学研究-学术会议
*/
@Slf4j
@Service
public class ResearchMeetingServiceImpl implements ResearchMeetingService {

    @Resource
    private ResearchMeetingMapper researchMeetingMapper;
    @Resource
    ...
    */
    @Override
    public ResponseResult insertSelective(ResearchMeeting record) {
        log.info("======= 插入ResearchMeeting数据 ======="+record);
        record.setId(idGenerator.nextId());
        record.setSubmitTime(new Date());
        if (researchMeetingMapper.insertSelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 插入ResearchMeeting成功 =======");
            return ResponseResult.ofSuccess("添加成功");
        }
        log.error("======= 插入ResearchMeeting失败 =======");
     ...
     */
    @Override
    public ResponseResult updateByPrimaryKeySelective(ResearchMeeting record) {
        log.info("======= 更新ResearchMeeting数据 ======="+record);
        if (researchMeetingMapper.updateByPrimaryKeySelective(record) == 1) {
            if(record.getNames() != null){
                for (String name : record.getNames()){
                    attachmentMapper.updateRecordIdByName(record.getId(), name);
                }
            }
            log.info("======= 更新ResearchMeeting成功 =======");
            return ResponseResult.ofSuccess("更新成功");
        }
        log.error("======= 更新ResearchMeeting失败 =======");
    ...
    */
    ...
    public ResponseResult findByUsername(String username) {
        log.info("======= 查找老师的所有ResearchMeeting记录 ======="+username);
        List<ResearchMeeting> records = researchMeetingMapper.findByUsername(username);
    ...
    public ResponseResult findByRecordId(Long recordId) {
        log.info("======= 查找单条ResearchMeeting记录 ======="+recordId);
        HashMap<String, Object> map = new HashMap<>();
        map.put("record", researchMeetingMapper.findById(recordId));
    ...
    public ResponseResult find() {
        log.info("======= 导出ResearchMeeting全部记录 =======");
        return ResponseResult.ofSuccess("查询全部记录",researchMeetingMapper.find());
    }

    @Override
    public ResponseResult deleteById(Long Id) {
        if(researchMeetingMapper.deleteById(Id) == 1){
